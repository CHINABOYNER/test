...
public class CollegeController {
     ...
     */
    @PostMapping
    public Result getCollegeData(@RequestHeader(value = "token", required = false) String token) {
        if(token == null) {
        ...
        for (UserInfo userInfo : ordinaryUserList) {
            if(myMap.get(userInfo.getCollege()) != null || userInfo.getType() != 1) {
                continue;
            } else {
            ...
            if(infoXXXByDate == null || userInfo.getType() != 1) {
                continue;
            }
    ...
    @PutMapping
    public Result getNotSubmitCollege(@RequestHeader(value = "token", required = false) String token) {
        if(token == null) {
            return new Result(201, "请先登录");
        }
        JSONObject parseToken = JWT.parse(token);
