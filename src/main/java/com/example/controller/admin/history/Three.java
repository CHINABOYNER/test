...
public class Three {
    ...
    @GetMapping
    public void downloadThree(HttpServletResponse httpServletResponse) throws IOException {
        Date today = new Date();
        SimpleDateFormat df=new SimpleDateFormat("yyyy-MM-dd");
        String yesterday = df.format(new Date(today.getTime() - 3 * 24 * 60 * 60 * 1000));
    ...
    @PostMapping
    public Result showThree(@RequestHeader(value = "token", required = false) String token) {
        if(null == token) {
